...
function ProcessImage($imgFile)
{
	...
	// Parse image
	list ($y_values, $i_values, $q_values) = ParseImage("images-small/".$imgFile);
	...
	// save y-coeffs
	InsertCoeffsToDB("coeffs_y",$y_trunc,$imageid,$connection);

	// save i-coeffs
	InsertCoeffsToDB("coeffs_i",$i_trunc,$imageid,$connection);

	// save q-coeffs
	InsertCoeffsToDB("coeffs_q",$q_trunc,$imageid,$connection);
...
// save coefficients
function InsertCoeffsToDB($dbtable,$coefftable,$imageid,$connection)
{
	global $COEFFNUM;
...
$db_server["password"] = "cumlaude2018"; // DB password
$db_server["database"] = "cbir_cars";// database name
